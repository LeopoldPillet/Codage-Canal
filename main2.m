...
awgn_channel = comm.AWGNChannel(...
    ...
    'SignalPower',1);
%% Decodeur
[ deco_viterbi_1,treillis1] = cpr_poly_vit();
...
%         rec_msg = double(Lc < 0); % Décision
        T_rx    = T_rx + toc(rx_tic);  % Mesure du débit de décodage
        
%         err_stat   = step(stat_erreur, msg, rec_msg); % Comptage des erreurs binaires
        %% Decodeur
        Un_1 = step(deco_viterbi_1,Lc);
        
        err_stat   = step(stat_erreur, msg, Un_1(1:end-6)); % Comptage des erreurs binaires
