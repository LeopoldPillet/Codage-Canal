...
for i=1:4
    ...
    for i_snr = 1:length(EbN0dB)
        ...
        treillis = poly2trellis(m(i)+1,[p(2*i-1),p(2*i)]);
        enco = comm.ConvolutionalEncoder(treillis,'TerminationMethod', 'Continuous');
        %% Construction decodeur
        deco = comm.ViterbiDecoder(treillis,...
            'TracebackDepth', 5*(m(i)+1),...
        ...
        while (err_stat(2) < nbr_erreur && err_stat(3) < nbr_bit_max)
            ...
            enc = step(enco,msg);
            %% Modulateur BPSK
            x      = step(mod_psk,  enc); % Modulation QPSK
            T_tx   = T_tx+toc(tx_tic);    % Mesure du débit d'encodage
...
%             rec_msg = Lc;
            ...
            %% Decodage
            Un = step(deco,Lc);
            err_stat   = step(stat_erreur, msg, Un(1:end)); % Comptage des erreurs binaires
    ...
    semilogy(EbN0dB,ber);
    hold all
    plot(EbN0dB,ones(1,length(EbN0dB))*10^-5);
    xlim([0 10])
    ylim([1e-6 1])
    grid on
    xlabel('$\frac{E_b}{N_0}$ en dB','Interpreter', 'latex', 'FontSize',14)
